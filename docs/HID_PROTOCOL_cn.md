...
**Report ID = `0xB3`,固定 12 字节,Output(主机→设备):**

| 偏移 | 长度 | 字段 | 说明 |
|------|------|------|------|
| [0]  | 1B   | Report ID | 固定 `0xB3` |
| [1]  | 1B   | 红色操作码 | RGB 红通道控制 |
| [2]  | 1B   | 红色亮度 | 0~255 |
| [3]  | 1B   | 绿色操作码 | RGB 绿通道控制 |
| [4]  | 1B   | 绿色亮度 | 0~255 |
| [5]  | 1B   | 蓝色操作码 | RGB 蓝通道控制 |
| [6]  | 1B   | 蓝色亮度 | 0~255 |
| [7]  | 1B   | HOME灯操作码 | HOME键 LED 控制 |
| [8]  | 1B   | HOME灯亮度 | 0~255 |
| [9]  | 1B   | 消费区操作码 | 消费按键区 LED 控制 |
| [10] | 1B   | 消费区亮度 | 固定亮度(不可调) |
| [11] | 1B   | 未使用 | 0x00 |
...
|------|----------|----------|------|----------|
| RGB 红 | byte[1] | byte[2] | RGB 子通道 | 是 (0~255) |
| RGB 绿 | byte[3] | byte[4] | RGB 子通道 | 是 (0~255) |
| RGB 蓝 | byte[5] | byte[6] | RGB 子通道 | 是 (0~255) |
| HOME键灯 | byte[7] | byte[8] | 单色 LED | 是 (0~255) |
| 消费区灯 | byte[9] | byte[10] | 单色 LED | 否 (固定亮度) |
...
```
# 纯红 (灭灯后)
B3 01 FF 02 00 02 00 02 00 02 00 00

# 纯绿 (灭灯后)
B3 02 00 01 FF 02 00 02 00 02 00 00

# 纯蓝 (灭灯后)
B3 02 00 02 00 01 FF 02 00 02 00 00

# 白色 (红+绿+蓝全开)
B3 01 FF 01 FF 01 FF 02 00 02 00 00

# 全灭
B3 02 00 02 00 02 00 02 00 02 00 00

# HOME灯全亮
B3 00 00 00 00 00 00 01 FF 00 00 00

# 消费区灯亮
B3 00 00 00 00 00 00 00 00 01 FF 00

# HOME灯 + 消费区灯全开
B3 00 00 00 00 00 00 01 FF 01 FF 00
```

> **注意:** hidapi `write()` 的第一个字节是 Report ID (`0xB3`),后跟 12 字节数据。
...
|------|------|------|------|
| [0]  | 1B   | Report ID | 固定 `0xB3` |
| [1]  | 1B   | 马达使能 | bit 位控制,见 §7.2 |
| [2]  | 1B   | 左马达强度 | 0~255 (脉冲模式有效) |
| [3]  | 1B   | 左马达持续时间 | 值 × 0.25 秒,0 = 脉冲模式 |
| [4]  | 1B   | 右马达强度 | 0~255 (脉冲模式有效) |
| [5]  | 1B   | 右马达持续时间 | 值 × 0.25 秒,0 = 脉冲模式 |
| [6-11] | 6B | 未使用 | 0x00 |

### 7.2 马达使能位 (byte[1])

byte[1] 使用 2-bit 编码分别控制左右马达:
...
**脉冲模式** (持续时间字节 = 0):
- 马达振动一下即停(极短脉冲)
- 强度由 byte[2]/byte[4] 控制,0~255 线性可调
- 适合快速连续发送实现自定义振动波形

**定时模式** (持续时间字节 > 0):
- 马达持续振动指定时间后自动停止
- 持续时间 = 字节值 × 0.25 秒 (如 0x04 = 1秒, 0x28 = 10秒)
- 定时模式下强度固定为最大,byte[2]/byte[4] 无效
- 可用停止命令(bits=10)提前终止
...
```
# 左马达脉冲 (最大强度)
B3 02 01 FF 00 00 00 00 00 00 00 00

# 右马达脉冲 (最大强度)
B3 02 04 00 00 FF 00 00 00 00 00 00

# 双马达脉冲 (最大强度)
B3 02 05 FF 00 FF 00 00 00 00 00 00

# 左马达定时 1 秒
B3 02 01 FF 04 00 00 00 00 00 00 00

# 右马达定时 2 秒
B3 02 04 00 00 FF 08 00 00 00 00 00

# 双马达定时 1 秒
B3 02 05 FF 04 FF 04 00 00 00 00 00

# 停止左马达
B3 02 02 00 00 00 00 00 00 00 00 00

# 停止右马达
B3 02 08 00 00 00 00 00 00 00 00 00

# 停止双马达
B3 02 0A 00 00 00 00 00 00 00 00 00
```

### 7.5 与 LED 命令的关系

LED 和振动共用 Report ID `0xB3`,通过 **byte[0]** 区分命令类型:
- `byte[0] = 0x01` → LED 控制 (见 §6)
- `byte[0] = 0x02` → 振动控制 (本节)
